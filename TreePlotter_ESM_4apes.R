...
RESULTS_ROOT <- "/home/rohan/PLM-Phylo/Results/Trees/ESM_35M_Trees/Attention_Trees/OMAM20/OMAM20_4apes/Trees"

# Updated per your note
REF_TREE_PATH <- "/home/rohan/PLM-Phylo/Data/Trees/OMAM_supertree_pruned_34spp.nwk"

# Output folder
OUT_DIR <- "/home/rohan/PLM-Phylo/Plots/ESM_35M"
dir.create(OUT_DIR, recursive = TRUE, showWarnings = FALSE)

# Put per-protein plots in their own folder (cleaner)
PER_PROTEIN_DIR <- file.path(OUT_DIR, "ESM_4apes_layerhead_correctness_per_protein")
...
for (domain_dir in domain_dirs) {
  ...
  inner <- file.path(domain_dir, domain)  # e.g. TF_4apes/TF_4apes
...
# Write CSV
write_csv(protein_summary, file.path(OUT_DIR, "ESM_4apes_layerhead_correct.csv"))
...
# =========================
# ASTRAL final-consensus correctness (one check per protein)
# =========================

# Finds a single "best" astral consensus tree for a given domain.
# Priority:
#   1) domain_dir/<domain>_astral_output.nwk
#   2) inner/<domain>_astral_output.nwk
#   3) anything matching *_astral_output.nwk or *_astral_consensus*.nwk under domain_dir (recursive)
find_astral_tree <- function(domain_dir, domain) {
  inner <- file.path(domain_dir, domain)
  
  # Common expected filenames
  cands_priority <- c(
    file.path(domain_dir, paste0(domain, "_astral_output.nwk")),
    file.path(inner,     paste0(domain, "_astral_output.nwk")),
    file.path(domain_dir, paste0(domain, "_astral_consensus.nwk")),
    file.path(inner,     paste0(domain, "_astral_consensus.nwk"))
  )
  cands_priority <- cands_priority[file.exists(cands_priority)]
  if (length(cands_priority) > 0) return(cands_priority[[1]])
  
  # Fallback: recursive search for common patterns
  cands <- list.files(
    domain_dir,
    recursive = TRUE,
    full.names = TRUE,
    pattern = "(_astral_output\\.nwk$|_astral_consensus.*\\.nwk$)",
    ignore.case = TRUE
  )
  ...
  # - prefer paths with fewer directory levels (closer to root)
  # - if tie, prefer shorter path string
  depth <- lengths(strsplit(cands, .Platform$file.sep, fixed = TRUE))
  cands <- cands[order(depth, nchar(cands))]
  return(cands[[1]])
}
...
for (domain_dir in domain_dirs) {
  ...
  astral_path <- find_astral_tree(domain_dir, domain)
  if (is.na(astral_path) || !file.exists(astral_path)) {
    astral_rows[[length(astral_rows) + 1]] <- tibble(
      domain = domain,
      astral_tree = NA_character_,
      rf_astral = NA_real_,
      astral_correct = NA
    )
    next
  ...
  tr_astral <- standardize_tree(read.tree(astral_path))
  rfA <- rf_unrooted(tr_astral, ref_tree)
  correctA <- ifelse(is.na(rfA), NA, rfA <= CORRECT_RF_THRESHOLD)
  
  astral_rows[[length(astral_rows) + 1]] <- tibble(
    domain = domain,
    astral_tree = astral_path,
    rf_astral = rfA,
    astral_correct = correctA
  )
}

astral_df <- bind_rows(astral_rows) %>%
  arrange(desc(astral_correct), domain)

# Write per-protein ASTRAL correctness CSV
write_csv(astral_df, file.path(OUT_DIR, "ESM_4apes_astral_consensus_correctness.csv"))

# Print per-protein ASTRAL correctness table
cat("\nASTRAL final-consensus correctness (per protein):\n")
print(astral_df)

# Overall ASTRAL summary counts (right vs wrong)
astral_summary <- astral_df %>%
  summarize(
    n_proteins = n(),
    n_scored   = sum(!is.na(astral_correct)),
    n_correct  = sum(astral_correct == TRUE,  na.rm = TRUE),
    n_incorrect= sum(astral_correct == FALSE, na.rm = TRUE),
    pct_correct= ifelse(n_scored > 0, 100 * n_correct / n_scored, NA_real_)
  )

write_csv(astral_summary, file.path(OUT_DIR, "ESM_4apes_astral_consensus_overall_summary.csv"))

cat("\nASTRAL final-consensus overall summary:\n")
print(astral_summary)

# (Optional) merge into your existing protein_summary table for one combined CSV
protein_summary_with_astral <- protein_summary %>%
  left_join(
    astral_df %>% select(domain, rf_astral, astral_correct),
    by = "domain"
  )

write_csv(
  protein_summary_with_astral,
  file.path(OUT_DIR, "ESM_4apes_layerhead_correct_with_astral.csv")
)
...
p_all <- ggplot(df, aes(x = head, y = layer_num, fill = as.character(correct))) +
  ...
  scale_y_reverse(breaks = c(1,5,10,15,20,25,30,33)) +
  labs(title = "esm-pFM 4apes: Correct (green) vs Incorrect (red)", x = "Head", y = "Layer") +
  theme_minimal(base_size = 11) +
  theme(panel.grid = element_blank()) +
  facet_wrap(~ domain, ncol = 4)

ggsave(
  filename = file.path(OUT_DIR, "ESM_4apes_layerhead_correctness.png"),
